// 마이페이지 구장좋아요리스트
const getMyFieldLikeList = (email:string, pageStart:number, pageEnd: number) =>{
...
const getMyReviewList = (email:string, pageStart:number, pageEnd: number) =>{
    let sql = `SELECT review_id,
                ...
                update_datetime
            FROM field_review
            where email = '` + email + `' order by a.insert_datetime desc `;
        if(pageStart && pageEnd){
            sql = sql + ' limit ' + pageStart + ', ' + pageEnd;
        }
    return sql;
...
//마이페이지 커뮤니티 리스트
const getMyCommunityList = (email:string, pageStart:number, pageEnd: number) => {
...
// 마이페이지 커뮤니티관심리스트
const getMyCommunityWishList = (email:string, pageStart:number, pageEnd: number) =>{
    let sql ='SELECT distinct commu_id' +
            ', commu_title ' +
            ', commu_txt ' +
            ', email ' +
            ', area ' +
...
module.exports = {getMyFieldLikeList, getMyReviewList, getMyCommunityList, getMyCommunityWishList};