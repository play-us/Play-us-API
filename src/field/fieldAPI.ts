...
field.get(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      let sql = fieldDB.getFieldLike(fieldId, email, pageStart, pageEnd);
      const rows = await db.query(sql);
      const conn = await db.getConnection();
      conn.release();
      if (rows) return res.status(200).json({ result: camelsKeys(rows[0]) });
...
field.get(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      let sql = fieldDB.getReservation(
        resvId,
        fieldId,
        email,
        resvDate,
...
field.post(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      let sql = fieldDB.insertReservation(
        fieldId,
        email,
        resvDate,
        resvStartTime,
...
field.delete(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      let sql = fieldDB.updateReservationCancel(resvId);
      const rows = await db.query(sql);
      const conn = await db.getConnection();
      conn.release();
      if (rows) return res.status(200).json({ result: camelsKeys(rows[0]) });
